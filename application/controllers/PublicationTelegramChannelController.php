...
class PublicationTelegramChannelController extends TA_Controller
{

	public function __construct()
	{
		parent::__construct();
		parent::checkPermission("publication_telegram_channel");

		$this->load->model('PublicationTelegramChannel_model', 'PublicationTelegramChannel_model', TRUE);
		$this->lang->load('telegram_lang', $this->session->userdata('language'));

		$this->load->helper('string');
		$this->load->helper('kanban_communication_helper.php');
		$this->load->helper('cancel_campaign_helper.php');
		$this->load->helper('responses_helper.php');
	}

	function Index()
	{
		$this->load->model('Channel_model', '', TRUE);
		$channels = $this->Channel_model->ListChannelAvailableWithStatus(31);

		$data['main_content'] = 'pages/publication/telegram/channel/find';
		$data['view_name'] = $this->lang->line("telegram_channel_header");
		$data['sidenav'] = array('');
		$data['channel'] = $channels;
		$data['topnav'] = array('search' => true);
		$data['js'] = array("publicationTelegramChannel.js");
		$data['css'] = array(
			"publicationTelegramChannel.css",
		);
	...
	function Get()
	{
		...
		$result = $this->PublicationTelegramChannel_model->Get($param);
	...
	function Check_date($date, $hour)
	{
		...
		if ($dateInformation < $current) {
			$this->form_validation->set_message('check_date', $this->lang->line("telegram_channel_check_date"));
			return false;
	...
	function Check_hour($hour, $date)
	{
		...
		if ($hourInformation <= $current) {
			$this->form_validation->set_message('check_hour', $this->lang->line("telegram_channel_check_hour"));
			return false;
	...
	function Check_date_validity($date_start_validity, $date_and_times)
	{
		...
		if ($dateValidity <= $dateInformation) {
			$this->form_validation->set_message('check_date_validity', $this->lang->line("telegram_channel_check_date_validity"));
			return false;
		} else {
			return true;
		}
	...
	function Check_hour_validity($time_start_validity, $date_and_times)
	{
		...
		if ($hourValidity <= $hourInformation) {
			$this->form_validation->set_message('check_hour_validity', $this->lang->line("telegram_channel_check_hour_validity"));
			return false;
		} else {
			return true;
		}
	...
	{
		$data = $this->PublicationTelegramChannel_model->CheckTimeToEdit($token);

		json_response($this, 200, $data);
	}
	...
	function Save($token)
	{
		...
		if (isset($data["edit"]) && $data["edit"] === "expire") {
			if (isset($data['toggle_validity'])) {
				$this->form_validation->set_rules('date_start_validity', $this->lang->line("telegram_channel_time_start_validity"), 'required');
				$this->form_validation->set_rules('time_start_validity', $this->lang->line("telegram_channel_hour_start_validity"), 'required');
			} else {
				$data['date_start_validity'] = "";
				$data['time_start_validity'] = "";
			}
			$this->PublicationTelegramChannel_model->EditCampaignExpire($token, $data);
			redirect("/publication/telegram/channel", 'refresh');
		}

		if ($data["type_broadcast"] != "") {

			$this->form_validation->set_rules('input_title', $this->lang->line("telegram_channel_title"), 'trim|required|max_length[100]');
			$this->form_validation->set_rules('date_start', $this->lang->line("telegram_channel_date_scheduling"), 'required|callback_check_date[' . $data['time_start'] . ']');
			$this->form_validation->set_rules('time_start', $this->lang->line("telegram_channel_hour_scheduling"), 'required');
			$this->form_validation->set_rules('others[]', $this->lang->line("telegram_channel_select_channel"), 'required');

			if (isset($data['toggle_validity'])) {
				$this->form_validation->set_rules('date_start_validity', $this->lang->line("telegram_channel_time_start_validity"), 'required|callback_check_date_validity[' . $data['time_start_validity'] . "," . $data['date_start'] . "," . $data['time_start'] . ']');
				$this->form_validation->set_rules('time_start_validity', $this->lang->line("telegram_channel_hour_start_validity"), 'required|callback_check_hour_validity[' . $data['date_start_validity'] . "," . $data['date_start'] . "," . $data['time_start'] . ']');
			} else {
				$data['date_start_validity'] = "";
				$data['time_start_validity'] = "";
			}
			...
			if ($data["type_broadcast"] == "text") {
				...
				$this->form_validation->set_rules('input-data', $this->lang->line("telegram_channel_validation_message"), 'trim|required|min_length[1]|max_length[1024]');
			...
			if ($data["type_broadcast"] == "image") {
				...
				$this->form_validation->set_rules('file0', $this->lang->line("telegram_channel_validation_img"), 'trim|required');
			...
			if ($this->form_validation->run() == false) {
				...
				if ($token != '0') {
					$ret = $this->PublicationTelegramChannel_model->Edit($token, $data);
					...
					$ret = $this->PublicationTelegramChannel_model->Add($data);
	...
	{
		$this->load->model('Channel_model', 'Channel_model', TRUE);
		$channels = $this->Channel_model->ListChannelAvailableWithStatus(31);

		$queryChannel = $this->PublicationTelegramChannel_model->queryChannel();
		$queryLastBroadcast = $this->PublicationTelegramChannel_model->queryLastBroadcast();
		$queryExpireBroadcast = $this->PublicationTelegramChannel_model->queryExpireBroadcast();
		...
		$data['main_content'] = 'pages/publication/telegram/channel/add';
		$data['sidenav'] = array('');
		$data['Channels'] = $channels;
		$data['Groups'] = $group;
		$data['queryChannel'] = $queryChannel;
		$data['view_name'] = $this->lang->line("telegram_channel_waba_add");
		$data['topnav'] = array('search' => false, 'header' => true);
		$data['js'] = array("publicationTelegramChannel.js");
		$data['css'] = array(
			"broadcast" => "publicationTelegramChannel.css",
		...
		$data['selected_groups'] = $recover == "NULL" ? "" : $this->PublicationTelegramChannel_model->ListGroups($recover["others"]);
	...
	function View($token)
	{
		...
		$data['data'] = $this->PublicationTelegramChannel_model->View($token)[0];
		$data['data']['view'] = 'true';
		$data['channel'] = $this->Channel_model->List();
		$data['log'] = $this->PublicationTelegramChannel_model->getScheduleLog($token);

		$data['main_content'] = 'pages//publication/telegram/channel/view';
		$data['view_name'] = $this->lang->line("telegram_channel_view");
		$data['sidenav'] = array('');
		$data['topnav'] = array('search' => false, 'header' => true);
		$data['js'] = array("PublicationTelegramChannel.js");
		$data['css'] = array("PublicationTelegramChannel.css", "msFmultiSelect.css");
	...
	function Edit($token)
	{
		$data['data'] = $this->PublicationTelegramChannel_model->GetBroadcastByToken($token);
		$info = $this->PublicationTelegramChannel_model->CheckTimeToEdit($token);
		$data['data']['edit'] = isset($info["success"]) ? 'true' : 'expire';

		$data['type_broadcast'] = $data['data']['media_type'] == 1 ? "text" : "image";
		$data['main_content'] = 'pages//publication/telegram/channel/edit';
		$data['view_name'] = $this->lang->line("telegram_channel_waba_edit");
		$data['sidenav'] = array('');
		$data['topnav'] = array('search' => false, 'header' => true);
		$data['js'] = array("PublicationTelegramChannel.js");
		$data['css'] = array(
	...
	{
		$info = $this->PublicationTelegramChannel_model->ViewEdit($key_id);
	...
	{
		return $this->PublicationTelegramChannel_model->UpdateApprovalLog($data);
	}
	...
	function ResendApproval()
	{
		...
		$this->PublicationTelegramChannel_model->updateToResendEmail($data);

		$data_contacts = $this->PublicationTelegramChannel_model->getJsonMessage($data);
	...
	{
		$data = $this->input->post();
		$groups = $this->PublicationTelegramChannel_model->ListGroups($data);
	...
	function Cancel($token)
	{
		...
		if (isset($data["success"])) {
			CreateBroadcastLog($this->PublicationTelegramChannel_model->getIdBroadcastByToken($token), 4);

			$kanban_communication = new stdClass();
			$kanban_communication->token = $token;
			$kanban_communication->key_remote_id = $this->PublicationTelegramChannel_model->getChannelKeyRemoteIdByToken($token);
	...
	{
		$data = $this->input->post();
		$this->PublicationTelegramChannel_model->prepareMessageBusiness($data);
	}
	...
	{
		$data = $this->input->post();
		$this->PublicationTelegramChannel_model->pauseBroadcast($data);
		
		foreach ($data['tokens'] as $token) {
			CreateBroadcastLog($this->PublicationTelegramChannel_model->getIdBroadcastByToken($token), 2);
	...
	{
		$data = $this->input->post();
		$this->PublicationTelegramChannel_model->resumeBroadcast($data);
		
		foreach ($data['tokens'] as $token) {
			CreateBroadcastLog($this->PublicationTelegramChannel_model->getIdBroadcastByToken($token), 3);
	...
	{
		$data = $this->PublicationTelegramChannel_model->ResendBroadcast($token);
