...
class MercadoLivre_model extends TA_model
{
    ...
    function register($code, $pw)
    {
        ...
        $sql .= "from channel\n";
        $sql .= "where channel.type = 7 and channel.status = 2\n";
        ...
        $this->talkall_admin->where(array('type' => 7, 'id_company' => $this->session->userdata('id_company')));
        $this->talkall_admin->delete('channel');
        $this->talkall_admin->insert('channel', $values);

        return "config/edit/" . $channel_id;
    }
}
