...
class PublicationInstagramPost_model extends TA_model
{
    ...
    function Get($param)
    {
        ...
        $this->db->where("broadcast_schedule.is_Ig_publication", 1);
        ...
        $this->db->group_by("broadcast_schedule.id_broadcast_schedule, channel, schedule, creation, id_channel, creation, data, title, token, expire, status, config.timezone");
    ...
    {
        $this->db->select("DATE_FORMAT(DATE_ADD(from_unixtime(max(broadcast_schedule.schedule)), INTERVAL config.timezone HOUR), '%d/%m/%Y') AS date");
        $this->db->select("DATE_FORMAT(DATE_ADD(from_unixtime(max(broadcast_schedule.schedule)), INTERVAL config.timezone HOUR), '%H:%i') AS time");
        ...
        $this->db->where('broadcast_schedule.is_fb_publication', 2);
        $this->db->where('broadcast_schedule.is_Ig_publication', 1);
