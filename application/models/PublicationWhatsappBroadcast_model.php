...
class PublicationWhatsappBroadcast_model extends TA_model
{
        ...
        foreach ($data["others"] as $row) {
            ...
            foreach ($data["text"] as $key => $value) {
                ...
                if ($data["select_segmented_group"] == 0 && $data["select_segmented"] == 0) {
                    ...
                    $this->db->select("GROUP_CONCAT(DISTINCT contact.key_remote_id ORDER BY CASE WHEN contact_order IS NOT NULL THEN 0 ELSE 1 END, 
                    CASE WHEN contact_order IS NOT NULL THEN contact_order ELSE NULL END, creation) as participants", FALSE);
                    ...
                    $this->db->not_like("contact.key_remote_id", "@");
                ...
                if (isset($data['isPartial']) == 1) {
                    ...
                    $getIdWork = $this->getChannels($data['id_channel']);
                    ...
                    $hourStart = $this->getTimezoneChannel($data['date_start'], rtrim($hours['start'], ":00"), $data['id_channel']);
                    $hourEnd =  $this->getTimezoneChannel($data['date_start'], rtrim($hours['end'], ":00"), $data['id_channel']);
                    $typeLog = 6;
                }
                ...
                $values = [
                    ...
                    'data' => $data['text'][$key],
                    'expire' => $this->getTimezoneChannel($data['date_start_validity'], $data['time_start_validity'], $data['id_channel']),
    ...
    function countBroadcasts($param)
    {
        if (!empty($param['channel']) && is_string($param['channel'])) {
            $param['channel'] = explode(',', $param['channel']);
        }
        ...
        $this->db->like(
            "LOWER(CONCAT(broadcast_schedule.title, ' ', channel.id))",
            $this->db->escape_like_str(strtolower($param['text']))
        );
        ...
        switch ($param['status']) {
            ...
            case 6:
                ...
                break;
            default:
                $this->db->where_in("broadcast_schedule.status", array(1, 3, 6));
                break;
        }
        ...
        $this->db->select("broadcast_schedule.token, broadcast_schedule.status");
        $this->db->from("broadcast_schedule");
        $this->db->join("channel", "broadcast_schedule.id_channel = channel.id_channel", "inner");
        $this->db->join("config", "channel.id_channel = config.id_channel", "inner");
        $this->db->where("broadcast_schedule.is_wa_broadcast", 1);
        ...
        $tokens = array_column($broadcasts, 'token');
        $status = array_column($broadcasts, 'status');

        return [
            'countBroadcast' => $count,
            'tokens'         => $tokens,
            'status'         => $status
        ];
    }
