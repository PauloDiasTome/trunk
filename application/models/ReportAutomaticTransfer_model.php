...
class ReportAutomaticTransfer_model extends TA_model
{
    function Get($text, $sector, $dt_start, $dt_end, $start, $length, $order_column, $order_dir)
    {
        ...
        $sql = "SELECT 
                ...
                WHERE
                    contact.spam = 1 AND contact.deleted = 1 AND wait_list.status = 1 AND
                        CONCAT(contact.full_name, ' ',contact.key_remote_id, ' ', user.name) LIKE '%{$text}%'\n";
    ...
    function ExportCsv($text, $dt_start, $dt_end, $channels)
    {
        $text = trim($text);
