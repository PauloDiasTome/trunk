...
class ReportPrincessFields_model extends TA_model
{
    ...
    function Get($text, $dt_start, $dt_end, $start, $length, $order_column, $order_dir)
    {
        $sql = "SELECT 
                ...
                WHERE
                     chat_list.is_bot = 1 AND contact.deleted = 1 AND

                CONCAT(contact.full_name, ' ', contact.key_remote_id) LIKE '%{$text}%' \n";

        if ($dt_start != "") {
            $sql .=  "AND DATE_FORMAT(DATE_ADD(from_unixtime(chat_list.last_timestamp_client), INTERVAL config.timezone HOUR),'%Y-%m-%d') between '$dt_start' and '$dt_end' \n";
        }

        switch ($order_column) {
            case 0:
                $sql .= "order by contact.creation $order_dir\n";
                break;
    ...
    function Count($text, $dt_start, $dt_end)
    {
        $sql = "SELECT 
                ...
                FROM
                    ...
                    config on channel.id_channel = config.id_channel
                WHERE
                    DATE_FORMAT(DATE_ADD(from_unixtime(chat_list.last_timestamp_client), INTERVAL config.timezone HOUR),'%Y-%m-%d') between '$dt_start' and '$dt_end'
    ...
    function History($id, $creation)
    {
        $sql = "SELECT 
                    * from messages
                 WHERE 
                    messages.id_chat_list = '$id'\n";
        if ($creation != 0) {

            $sql .= "AND messages.creation < '$creation'  
                        ORDER BY messages.creation DESC, messages.id_message DESC LIMIT 0,40\n";
        } else {
            $sql .= "ORDER BY messages.creation DESC, messages.id_message DESC LIMIT 0,40\n";
        }
