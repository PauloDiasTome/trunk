...
class SantaEmiliaBot_model extends TA_model
{
	public $database;
	...
	function AddTheWaitList($data)
	{ // Envia para o atendimento adicionando na fila de espera(table = wait_list)
		$date = new DateTime();
	...
	function CheckWaitList($data)
	{ // Confirma presença na fila
	...
	function Office_Hours_End($id)
	{
		...
		if (date('l') === 'Saturday' || date('l') === 'Sunday' || $currentHour < $startHour || $currentHour > $endHour) {
			...
			$sql = "select office_hours_end from config where id_channel = 222";
