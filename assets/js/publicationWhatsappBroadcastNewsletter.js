...
function createBroadcast(data) {
    ...
    textArea.placeholder = GLOBAL_LANG.whatsapp_broadcast_newsletter_description_placeholder;
    ...
    text_caracter.textContent = GLOBAL_LANG.whatsapp_broadcast_newsletter_text_caracter;
...
function callAlert(verify) {
    ...
    switch (verify) {
        case "limit":
            swal({
                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_title,
                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_text,
                type: "warning",
                confirmButtonColor: '#3085d6',
                confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_confirmButtonText,
                cancelButtonClass: "btn btn-secondary"
            });
            break;
        case "maxSize":
            swal({
                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_two_title,
                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_two_text,
                type: "warning",
                confirmButtonColor: '#3085d6',
                confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_two_confirmButtonText,
                cancelButtonClass: "btn btn-secondary"
            });
            break;
        case "png":
            swal({
                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_three_title,
                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_three_text,
                type: "warning",
                confirmButtonColor: '#3085d6',
                confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_three_confirmButtonText,
                cancelButtonClass: "btn btn-secondary"
            });
            break;
        case "extensions":
            swal({
                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_three_title,
                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_three_extensions,
                type: "warning",
                confirmButtonColor: '#3085d6',
                confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_dropzone_three_confirmButtonText,
                cancelButtonClass: "btn btn-secondary"
...
function checkFilter() {
    if (Filters) {
        ...
        if (Filters.newsletter.search) {
            document.getElementById("search").value = Filters.newsletter.search;
        }

        if (Filters.newsletter.input_search) {
            document.getElementById("input-search").value = Filters.newsletter.input_search;
        }

        if (Filters.newsletter.channel.length !== 0) {
            modalFilter();
            document.getElementById("check-select2").click();

            document.querySelectorAll("#mult-select2 .cust_").forEach((element, index) => {

                if (Filters.newsletter.channel.includes(element.value))
                    document.querySelectorAll("#mult-select2 .cust_")[index].click();
            })
        }

        if (Filters.newsletter.status) {
            modalFilter();
            document.getElementById("check-status").click();
            document.getElementById("select-status").value = Filters.newsletter.status;
        }

        if (Filters.newsletter.dt_start) {
            modalFilter();
            ...
            document.getElementById("dt-start").value = Filters.newsletter.dt_start;
            document.getElementById("dt-end").value = Filters.newsletter.dt_end;
        }
...
function find() {
    $('#datatable-basic').DataTable({
        ...
        ajax: {
            ...
            data: {
                text: $("#search").val(),
                channel: $('#verify-select').val() == "2" ? "" : $('#multiselect2').val(),
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                    ...
                    let ret = `<span class="badge badge-sm badge-secondary">${GLOBAL_LANG.whatsapp_broadcast_newsletter_datatable_column_status_processing}</span>`

                    switch (full.status) {
                        case '1':
                        case '6':
                            ret = `<span class="badge badge-sm badge-info">${GLOBAL_LANG.whatsapp_broadcast_newsletter_datatable_column_status_sending}</span>`
                            break;
                        case '2':
                            ret = `<span class="badge badge-sm badge-success">${GLOBAL_LANG.whatsapp_broadcast_newsletter_datatable_column_status_send}</span>`
                            break;
                        case '3':
                            ret = `<span class="badge badge-sm badge-secondary">${GLOBAL_LANG.whatsapp_broadcast_newsletter_datatable_column_status_processing}</span>`
                            break;
                        case '5':
                            ret = `<span class="badge badge-sm badge-danger">${GLOBAL_LANG.whatsapp_broadcast_newsletter_datatable_column_status_called_off}</span>`
                            break;
                        default:
                            ret = ret
                            break;
                    ...
                    return `<div class="dropleft">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="multiDropdownMenu">
                                            <a id="` + full.token + `" class="dropdown-item table-action-view" style="cursor: pointer">
                                                ...
                                                <span>${GLOBAL_LANG.whatsapp_broadcast_newsletter_datatable_column_action_view}</span>
                                            </a>
                                            <a id="` + full.token + `" class="dropdown-item ${full.status == 4 ? 'table-action-deleted disabled' : (full.status == 5 ? 'table-action-deleted disabled' : (full.status == 2 ? 'table-action-deleted disabled' : 'table-action-delete'))}"  style="cursor: pointer">
                                                ...
                                                <span>${GLOBAL_LANG.whatsapp_broadcast_newsletter_datatable_column_action_cancel}</span>
                                            </a>
                                            <a id="${full.token}" class="dropdown-item table-action-resend"  
                                                style="cursor: pointer; display:${(full.status != 3 && full.media_type != 33) ? containsSupport(USER_EMAIL) == true ? 'block' : 'none' : 'none'}">
                                                <div style="width: 24px; display: inline-block"> 
                                                    <i class="fas fa-redo-alt" style="font-size: 12pt; margin-left: 3px"></i>
                                                </div>
                                                <span>${GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_resend}</span>
                                           </a>
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (url.includes("/edit/")) {
                ...
                const response = await fetch(`${document.location.origin}/publication/whatsapp/broadcast/newsletter/checktime/edit/${token}`);
...
async function submit() {
    ...
    const input_title = formValidation({ field: document.getElementById("input_title"), text: GLOBAL_LANG.whatsapp_broadcast_newsletter_valid_title, required: true, min: 3, max: 100 });
    const select_channel = formValidation({ field: document.getElementById("multiselect"), text: GLOBAL_LANG.whatsapp_broadcast_newsletter_valid_channel, required: true });
    ...
    switch (document.getElementById("type_broadcast").value) {
        case "text":
            form_validation = formValidation({ field: document.getElementById("input-data"), text: GLOBAL_LANG.whatsapp_broadcast_newsletter_validation_message, required: true, min: 1, max: 1024 });
            break;
        case "media":
    ...
    if (input_title && select_channel && form_validation) {
...
function alertEditDatatable(status) {
    ...
    switch (status) {
        case '2':
            message = GLOBAL_LANG.whatsapp_broadcast_newsletter_datatables_edit_status_2
            break;
        case '5':
            message = GLOBAL_LANG.whatsapp_broadcast_newsletter_datatables_edit_status_5
            break;
        case '1':
        case '6':
            message = GLOBAL_LANG.whatsapp_broadcast_newsletter_datatables_edit_status_6
            break;
        default:
            message = GLOBAL_LANG.whatsapp_broadcast_newsletter_datatables_edit_default
            break;
    }
...
function alertErrors(data) {
    if (data.errors?.code == "TA-023") {
        swal({
            title: GLOBAL_LANG.whatsapp_broadcast_newsletter_error_title,
            text: `${GLOBAL_LANG.whatsapp_broadcast_newsletter_error_ta023} (${data.errors.code})`,
            type: "error",
            buttonsStyling: !1,
    ...
    if (data.errors?.code == "TA-024") {
        swal({
            title: GLOBAL_LANG.whatsapp_broadcast_newsletter_error_title,
            text: `${GLOBAL_LANG.whatsapp_broadcast_newsletter_error_ta024} (${data.errors.code})`,
            type: "error",
            buttonsStyling: !1,
            confirmButtonClass: "btn btn-danger"
        });
...
$(document).ready(function () {
    ...
    if (multiselectAdd != null && !edit) {
        ...
        new MSFmultiSelect(
            ...
            width: "100%",
            height: 47,
            placeholder: GLOBAL_LANG.whatsapp_broadcast_newsletter_placeholder_select_channel,
        });
        ...
        $("#datatable-basic").on("click", ".table-action-view", function () {
            window.location.href = "newsletter/view/" + this.id;
        });

        $("#datatable-basic").on("click", ".table-action-edit", function () {
            window.location.href = "newsletter/edit/" + this.id;
        });

        $("#datatable-basic").on("click", ".table-action-resend", function () {
            swal({
                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_resend_title,
                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_resend_text,
                type: "warning",
                showCancelButton: !0,
                buttonsStyling: !1,
                reverseButtons: true,
                confirmButtonClass: "btn btn-success",
                confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_resend_confirmButtonText,
                cancelButtonClass: "btn btn-danger",
                cancelButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_resend_cancelButtonText,
            }).then(t => {
                if (t.value == true) {
                    $.post(document.location.origin + "/publication/whatsapp/broadcast/newsletter/resend/" + this.id, function (data) {
                        if (data.success?.status == true) {
                            t.value && swal({
                                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_resend_two_title,
                                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_resend_two_text,
                                type: "success",
                                buttonsStyling: !1,
                                confirmButtonClass: "btn btn-success"
                            });
        ...
        $("#btn-cancel").on("click", function () {
            ...
            swal({
                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_group_delete_title,
                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_group_delete_text,
                type: "warning",
                showCancelButton: !0,
                buttonsStyling: !1,
                reverseButtons: true,
                confirmButtonClass: "btn btn-success",
                confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_group_delete_confirmButtonText,
                cancelButtonClass: "btn btn-danger",
                cancelButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_group_delete_cancelButtonText,
            }).then(t => {
                if (t.value == true) {
                    $.ajax({
                        type: "POST",
                        url: document.location.origin + "/publication/whatsapp/broadcast/newsletter/cancelgroup",
                        data: tokens,
                        cache: false,
                        contentType: false,
                        processData: false,
                        success: function (data) {
                            ...
                            if (data.errors?.code == "TA-027") {
                                t.value && swal({
                                    title: GLOBAL_LANG.whatsapp_broadcast_newsletter_validation_cancel_title,
                                    text: GLOBAL_LANG.whatsapp_broadcast_newsletter_validation_cancel_description,
                                    type: "error",
                                    buttonsStyling: !1,
                                    confirmButtonClass: "btn btn-danger"
                                });
                            ...
                            if (data.success?.status === true) {
                                t.value && swal({
                                    title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_group_delete_two_title,
                                    text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_group_delete_two_text,
                                    type: "success",
                                    buttonsStyling: !1,
                                    confirmButtonClass: "btn btn-success"
                                });
        ...
        $("#datatable-basic").on("click", ".table-action-delete", function () {
            ...
            swal({
                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_delete_title,
                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_delete_text,
                type: "warning",
                showCancelButton: !0,
                buttonsStyling: !1,
                reverseButtons: true,
                confirmButtonClass: "btn btn-success",
                confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_delete_confirmButtonText,
                cancelButtonClass: "btn btn-danger",
                cancelButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_delete_cancelButtonText,
            }).then(t => {
                if (t.value == true) {
                    $.post("newsletter/cancel/" + this.id, function (data) {
                        ...
                        if (data.success?.status === true) {
                            t.value && swal({
                                title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_delete_two_title,
                                text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_delete_two_text,
                                type: "success",
                                buttonsStyling: !1,
                                confirmButtonClass: "btn btn-success"
                            });
...
function createOption(amount, header, element = "") {
    ...
    const warningEqualOptions = (item) => {
        ...
        if (text == "") {
            item.style.borderBottom = "solid #007AFF 3.5px";
            item.parentElement.parentElement.querySelector('.alert-opt').style.display = "none";
            document.getElementById("text_preview" + item.id.split("input_")[1]).innerHTML = GLOBAL_LANG.whatsapp_broadcast_newsletter_message_poll_add;
        }
    }
    ...
    const addTextQuestionPreview = (item) => {
        if (item.value == "")
            document.getElementById("question_preview").innerHTML = GLOBAL_LANG.whatsapp_broadcast_newsletter_message_poll_question;
        else
    ...
    const setmultipleAnswers = (item) => {
        if (item.checked) {
            ...
            document.getElementById("preview-multiple-answers").innerHTML = GLOBAL_LANG.whatsapp_broadcast_newsletter_message_pollone_option_or_more;
        } else {
            document.getElementById("preview-multiple-answers-svg").src = "/assets/icons/kanban/concluded_dark_gray1.svg";
            document.getElementById("preview-multiple-answers").innerHTML = GLOBAL_LANG.whatsapp_broadcast_newsletter_message_poll_one_option;
        }
    }
    ...
    const headerPoll = () => {
        ...
        const question_text = Components.span({ text: GLOBAL_LANG.whatsapp_broadcast_newsletter_poll_question });
        const question_input = Components.input({ name: "question", value: element?.question ?? "", customAttribute: ["placeholder", GLOBAL_LANG.whatsapp_broadcast_newsletter_poll_question_placeholder, "maxlength", "255"] });
        const caracter = Components.span({ class: "count-caracter", text: "0", id: "count_caracter_question" });
        const title_text = Components.span({ text: GLOBAL_LANG.whatsapp_broadcast_newsletter_poll_option });
        ...
        const text = Components.span({ text: GLOBAL_LANG.whatsapp_broadcast_newsletter_poll_multiple_answers });
    ...
    const optionPoll = (option = "") => {
        ...
        const input = Components.input({ class: "opt", name: addOptionInput("option"), value: option || "", id: "input_" + uuid, customAttribute: ["placeholder", GLOBAL_LANG.whatsapp_broadcast_newsletter_poll_option_add, "maxlength", "100"] });
        const alert = Components.span({ class: "alert-opt", text: GLOBAL_LANG.whatsapp_broadcast_newsletter_poll_option_validion });
    ...
    const headerPreview = () => {
        ...
        const question = Components.div({ class: "question" });
        const question_span = Components.span({ id: "question_preview", text: element?.question ?? GLOBAL_LANG.whatsapp_broadcast_newsletter_message_poll_question });
        ...
        const multiple_answers_text = Components.span({ id: "preview-multiple-answers", text: (!element || element.multiple_answers) ? GLOBAL_LANG.whatsapp_broadcast_newsletter_message_pollone_option_or_more : GLOBAL_LANG.whatsapp_broadcast_newsletter_message_poll_one_option });
        const option = Components.div({ class: "option" });

        const show_voting = Components.div({ class: "show-voting" });
        const show_voting_span = Components.span({ text: GLOBAL_LANG.whatsapp_broadcast_newsletter_show_votes });
    ...
    const optionPreview = (option = "") => {
        ...
        const text_span = Components.span({ id: "text_preview" + uuid, text: option || GLOBAL_LANG.whatsapp_broadcast_newsletter_message_poll_add });
...
function modalFilter() {
    ...
    check_date.addEventListener("click", () => {
        if (check_date.checked) {
            ...
            dt_start.placeholder = GLOBAL_LANG.whatsapp_broadcast_newsletter_filter_period_placeholder_date_start;
...
function modalExport() {
    ...
    $.get(`/export/xlsx?
        ...
        &dt_start=${$('#dt-start').val()}
        &dt_end=${$('#dt-end').val()}
        &type=publicationWhatsappBroadcastNewsletter`, function (response) {
        Swal.fire({
            title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_export_title,
            text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_export_text,
            type: 'success',
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_export_confirmButtonText
        });
    });
}
...
function changeTypeWarning(type) {
    ...
    swal({
        title: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_change_type_title,
        text: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_change_type_text,
        type: "warning",
        showCancelButton: !0,
        buttonsStyling: !1,
        reverseButtons: true,
        confirmButtonClass: "btn btn-success",
        confirmButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_change_type_yes,
        cancelButtonClass: "btn btn-danger",
        cancelButtonText: GLOBAL_LANG.whatsapp_broadcast_newsletter_alert_change_type_no,
    }).then(t => {
        if (t.value == true) {
            switch (type) {
                case "media":
