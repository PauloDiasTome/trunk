...
let last_name_attendance;
let loading_top = false;
...
function message(json, position = "bottom") {
    ...
    if (last_date == '') {
        ...
        item.className = "date-floating " + textData.replace("/", "_").replace("/", "_");
    ...
    if (last_date != textData) {
        ...
        if (textData != undefined) {
            ...
            const date_show = document.querySelector('.date-floating .bottom span');
            date_show.innerText = textData;
    ...
    if (json.quoted_row_id != null && json.quoted != null) {
        ...
        switch (parseInt(json.quoted.media_type)) {
            ...
            case 35:
                ...
                var footer_inter = item_inter.interactive?.footer ? item_inter.interactive.footer.text : "";
    ...
    switch (parseInt(json.media_type)) {
        case 1:
            ...
            if (json.source_id != null) {
                ...
                if (json.quoted?.buttons == null) {
                    ...
                    if (json.quoted && json.quoted.media_type == 35) {
                        ...
                        span.innerHTML = `${GLOBAL_LANG.report_call_interactive_flow_message_client} ${parsedData.client ? GLOBAL_LANG.report_call_interactive_flow_message_yes : GLOBAL_LANG.report_call_interactive_flow_message_no}<br>CNPJ: ${parsedData.cnpj ?? ""}`;
                    } else {
            ...
            if (json.quoted?.buttons != null) {
                try {
                    ...
                    const buttons = JSON.parse(json.quoted.buttons);

                    // Extrai dados do flow
                    const fullNote = parsed.screen_0_Escolha_um_0?.split("_•_")[1] ?? "";
                    const note = fullNote.replace("(", "").replace(")", "") || "N/A";
                    const comment = parsed.screen_0_Deixe_um_comentrio_1 ?? "";

                    for (let btn of buttons) {

                        if (btn.type !== "FLOW") continue;
                        ...
                        subtitle.textContent = GLOBAL_LANG.report_call_template_flow_message_subtitle;
                        subtitle.style.fontSize = "13px";
                        subtitle.style.color = "#2b2b2bff";
                        ...
                        card.onclick = () => openFlowModal({
                            title: GLOBAL_LANG.report_call_template_flow_message_title_modal,
                            evaluationNote: note,
                            comment: comment
                        });
            ...
            if (json.source_id != null) {
                ...
                let span = document.createElement("span");
                if (json.quoted && json.quoted.media_type == 35)
                    span.innerHTML = `${GLOBAL_LANG.report_call_interactive_flow_message_client} ${JSON.parse(json.data).client ? GLOBAL_LANG.report_call_interactive_flow_message_yes : GLOBAL_LANG.report_call_interactive_flow_message_no}<br>CNPJ: ${JSON.parse(json.data).cnpj ?? ""}`;
                else
                    span.innerHTML = json.data;
        ...
        case 2:
            ...
            audioTranscribeIcon.title = GLOBAL_LANG.report_call_chat_audio_icon_title;
            ...
            switch (parseInt(json.key_from_me)) {
                case 1:
                    ...
                    message.innerHTML = `<svg xmlns='https://www.w3.org/2000/svg' viewBox='0 0 17 19'>
                                         ...
                                         <span>${GLOBAL_LANG.report_call_message_deleted}</span>`;
                    break;
                case 2:
                    ...
                    message.innerHTML = `<svg xmlns='https://www.w3.org/2000/svg' viewBox='0 0 17 19'>
                                         ...
                                         <span>${GLOBAL_LANG.report_call_deleted_message}</span>`;
                    break;
            }
            break;
        case 19:
        ...
        case 35:
            ...
            var footer_inter = data.interactive?.footer ? data.interactive.footer.text : '';
        ...
        case 32:
            ...
            if (json.media_mime_type === "image/jpeg") {
                ...
                caption_img.textContent = is_reply_story ? GLOBAL_LANG.report_call_story_reply : GLOBAL_LANG.report_call_story_mention;
            ...
            if (json.media_mime_type === "video/mp4") {
                ...
                caption_video.textContent = is_reply_story ? GLOBAL_LANG.report_call_story_reply : GLOBAL_LANG.report_call_story_mention;
    ...
    switch (parseInt(json.media_type)) {
        ...
        case 4:
            ...
            if (reaction) {
                if (json.key_from_me == 1)
                    item.className = 'item reaction _start';
                else
                    item.className = 'item reaction _end';
        ...
        case 20:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_call_chat_transfer_attendance + FormatShortTime(json.creation) + GLOBAL_LANG.report_call_chat_to;
            message.appendChild(span);
            item.appendChild(message);
            break;
        case 21:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_call_chat_start_attendance + FormatShortTime(json.creation);
        ...
        case 22:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_call_chat_waiting_attendance;
        ...
        case 23:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_call_chat_closed_attendance + FormatShortTime(json.creation);
        ...
        case 24:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_call_service_column_protocol + json.data;
    ...
    if (position === "top") {
        const first = chat.querySelector(".item");
        if (first) {
            chat.insertBefore(item, first);
        } else {
...
function find() {
    $('#datatable-basic').DataTable({
        ...
        ajax: {
            ...
            data: {
                text: $("#search").val(),
                channel: $('#verify-select-channel').val() == '2' ? 'NULL' : ($('#multiselect-channel').val().length == 0 ? 'NULL' : $('#multiselect-channel').val()),
                categories: $('#verify-select-categories').val() == '2' ? 'NULL' : ($('#multiselect-categories').val().length == 0 ? 'NULL' : $('#multiselect-categories').val()),
                label: $('#verify-select-label').val() == '2' ? 'NULL' : ($('#multiselect-label').val().length == 0 ? 'NULL' : $('#multiselect-label').val()),
                user: $('#select-user').val(),
                sector: $('#verify-select').val() == '2' ? 'NULL' : ($('#multiselect').val().length == 0 ? 'NULL' : $('#multiselect').val()),
                situation: parseInt($('#select-situation').val()),
        ...
        columnDefs: [
                ...
                render: function (data, type, full, meta) {
                    return `<div>
                                <span style="font-weight: bolder;">
                                    ${full.full_name.trim() ? full.full_name : full.key_remote_id.split("-")[0]}
                                </span>
                                <br>
                                <span>
                                    ${full.key_remote_id.split("-")[0]}
                                </span>
                            </div>`;
                }
            },
            {
                ...
                render: function (data, type, full, meta) {
                    if (full.labels) {
                        let tags = "";
                        const labels = full.labels.split(",");

                        for (let i = 0; i < labels.length; i++) {
                            const [name, color] = labels[i].split("/");

                            if (i < 3) {
                                tags += `<div class="chat-labels" style="cursor: pointer; background: ${color}">${truncateString(name, 10)}</div>`;
                                if (i == 2 && labels.length - 3 > 0) {
                                    tags += `<div class=count-labels>+ ${labels.length - 3} </div><br>`;
                                }
                            }
                        }
                        return tags;
            ...
            },
            {
                ...
                orderData: [6],
                render: function (data, type, full, meta) {
                    const rating = parseInt(full.rating);

                    if (!isNaN(rating) && rating >= 1 && rating <= 5) {
                        let stars = '';
                        for (let i = 1; i <= 5; i++) {
                            const hoverStyle = `onmouseover="this.style.transform='scale(1.2)'; this.style.filter='brightness(1.3)'"
                                   onmouseout="this.style.transform='scale(1)'; this.style.filter='brightness(1)'"`;

                            if (i <= rating) {
                                stars += `<span class="star-clickable star-auto-effect" 
                                style="color: #ffc107; font-size: 16px; cursor: pointer; transition: all 0.2s ease; display: inline-block;" 
                                ${hoverStyle}>★</span>`;
                            } else {
                                stars += `<span class="star-clickable star-auto-effect" 
                                style="color: #e0e0e0; font-size: 16px; cursor: pointer; transition: all 0.2s ease; display: inline-block;" 
                                ${hoverStyle}>☆</span>`;
                            }
                        }
                        ...
                        data-rating="${rating}" 
                        data-row-id="${full.id || meta.row}"
                        data-feedback="${full.feedback || ''}">
                        ${stars} 
                        <span class="star-clickable star-auto-effect" style="font-size: 12px; color:rgb(0, 28, 184);cursor: pointer;">(${rating}/5)</span>
                    </div>`;
                    } else if (full.feedback) {
                        let warningStars = '';
                        for (let i = 1; i <= 5; i++) {
                            warningStars += `<span class="star-clickable star-warning" 
                                style="color: #f0ad4e; opacity: 0.6; font-size: 16px; cursor: pointer; display: inline-block;">☆</span>`;
                        }

                        return `<div class="rating-stars-warning" 
                        data-row-id="${full.id || meta.row}"
                        data-feedback="${full.feedback || ''}"
                        data-status="processed-no-rating">
                        ${warningStars}
                        <span class="star-clickable" style="font-size: 12px; color: #f0ad4e; cursor: pointer;">(!)</span>
                    </div>`;
                    } else {
                        let emptyStars = '';
                        for (let i = 1; i <= 5; i++) {
                            emptyStars += `<span class="star-empty" 
                                style="color: #cccccc; opacity: 0.5; font-size: 16px; display: inline-block;">☆</span>`;
                        }

                        return `<div class="rating-stars-empty" 
                        data-row-id="${full.id || meta.row}">
                        ${emptyStars}
                        <span style="font-size: 12px; color: #999; opacity: 0.7;">(0/5)</span>
                    </div>`;
                    }
                },
            },
            {
                ...
                render: function (data, type, full, meta) {
                    switch (parseInt(full.status)) {
                        case 1:
                            return "<i class='fas fa-headset' title='" + GLOBAL_LANG.report_call_dt_columndefs_target7_title_edit + "' style='background-color: #11cdef;color: #fff;padding:4.5px 5px 5px 4.8px; border-radius: 30px;'></i> &nbsp" + getServiceTime(full);
                        case 2:
                            return "<i class='fas fa-check-circle' title='" + GLOBAL_LANG.report_call_dt_columndefs_target7_title_delete + "' style='background-color: #11935b;color: #fff;padding:4.5px 5px 4px 4.8px; border-radius: 30px;'></i> &nbsp" + getServiceTime(full);
                    }
                }
            },
            {
                ...
                render: function (data, type, full, meta) {
                    ...
                    return `<div class="dropleft">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="multiDropdownMenu">
                                        <a href='#' id="${full.id_chat_list}" data-name="${full.full_name.trim() == '' ? full.key_remote_id.split("-")[0] : full.full_name}" data-number="${full.key_remote_id}" data-status="${full.status}" data-protocol="${full.protocol}" data-creation="${full.creation}" class="dropdown-item table-btn-comment" style="cursor: pointer">
                                            <div style="width: 24px; display: inline-block"> 
                                                <i class="far fa-comments"></i>
                                            </div>
                                            <span>${GLOBAL_LANG.report_call_service_column_action_view}</span>
                                        </a>
                                        <a href='#' id="${full.id_chat_list}" class="dropdown-item exportTalkClass" style="cursor: pointer">
                                            <div style="width: 24px; display: inline-block"> 
                                                <i class="fas fa-file-export" style="font-size: 10pt; margin-left: 2pt"></i>
                                            </div>
                                            <span>${GLOBAL_LANG.report_call_service_column_action_export}</span>
                                        </a>
                                    </div>
                                </div>`
                }
...
function applyAutoStarEffect() {
    const stars = document.querySelectorAll('.star-auto-effect');
...
// ⭐ MUDANÇA: Agora a cada 15 segundos (15000ms)
setInterval(applyAutoStarEffect, 15000);

// Aplica o efeito imediatamente quando a página carrega
document.addEventListener('DOMContentLoaded', function () {
    setTimeout(applyAutoStarEffect, 2000);
});

function openRatingModal(rating, rowId, rowData, status) {
    let modalTitle, modalContent;

    if (rating > 0 || rating != null) {
        modalTitle = `<i class="fas fa-exclamation-triangle me-2"></i>${GLOBAL_LANG.report_call_assessment_unavailable || 'Avaliação Indisponível'}`;

        let starsDisplay = '';
        for (let i = 1; i <= 5; i++) {
            if (i <= rating) {
                starsDisplay += '<span style="color: #ffc107; font-size: 18px;">★</span>';
            } else {
                starsDisplay += '<span style="color: #f0ad4e; font-size: 18px; opacity: 0.6;">☆</span>';
            }
        }
                ...
                <div class="modal-content">
                    <div class="modal-header text-white">
                        <h5 class="modal-title">
                            <i class="fas fa-star me-2"></i>${GLOBAL_LANG.report_call_assessment_details}
                        </h5>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="fw-bold text-muted">${GLOBAL_LANG.report_call_assessment}</label>
                                <div class="mt-1">
                                    ${starsDisplay} 
                                    <span class="ms-2 badge bg-info">${rating}/5</span>
                                </div>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="fw-bold text-muted">${GLOBAL_LANG.report_call_AI_feedback}</label>
                            <div class="mt-2 p-3 border rounded bg-light" style="min-height: 80px;">
                                <em>${rowData.feedback || 'Nenhum feedback foi gerado pela IA.'}</em>
                            </div>
                        </div>
                        <div style="text-align:center;font-size:0.8rem;">
                           <cite>Resultado gerado por IA. Recomendamos verificação adicional.</cite>
                        </div>
                    </div>
                </div>
            </div>
    ...
    `;
        ...
        let feedback = '';
        switch (rowData.feedback) {
            case 'NO MESSAGES FOUND FOR EVALUATION':
                feedback = 'Não houve conversa suficiente para avaliar';
                break;
            default:
                feedback = 'Não houve conversa suficiente para avaliar';
                break;
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="mt-1">
                                    ${starsDisplay}<span style="color: #f0ad4e; font-size: 18px; opacity: 0.6;">(!) </span> 
                                    <span class="ms-2 badge">0/5</span>
                                </div>
                            </div>
                        </div>
                        <div class="mb-3">
...
$(document).on('click', '.star-clickable', function (e) {
    e.preventDefault();
    e.stopPropagation();

    const ratingContainer = $(this).closest('.rating-stars');
    const rating = ratingContainer.data('rating');
    const rowId = ratingContainer.data('row-id');
    const feedback = ratingContainer.data('feedback');

    const rowData = {
        feedback: feedback
    };

    openRatingModal(rating, rowId, rowData);
});
...
$(document).ready(function () {
    ...
    $("#datatable-basic").on("click", ".table-btn-comment", async function () {
        ...
        const timestamp = parseDatetimeTimestamp(this.dataset.creation);
        ...
        const data = await getMessages(get_messages);
        openChat(data, protocol);
    ...
    $(".chat").on("scroll", async function () {
        const chat = this;
        const is_at_top = chat.scrollTop <= 5;

        if (chat.scrollTop === 0 && chat.clientHeight === chat.scrollHeight) return;

        if (is_at_top && !loading_top) {
            loading_top = true;

            const first_item = document.querySelector(".chat .item");
            if (!first_item) return;

            const creation = first_item.dataset.index;
            const token_message = first_item.id;
            ...
            loading_top = false;
            const prev_height = chat.scrollHeight;

            data
                .filter(msg => msg.token !== token_message)
                .forEach(el => {

                    if (el.status == '0' && el.key_from_me == '2') return;

                    message(el, "top");

                    if (el.media_type === '19' && el.key_from_me === '1') {
                        el.user_name = el.data;
                        showUserName(el, true, 2);
                    }

                    if (el.key_from_me === '2') {
                        showUserName(el, true, 1);
                    }
                    removeDuplicateIds();
                });

            const new_height = chat.scrollHeight;
            chat.scrollTop = new_height - prev_height;

            $("#load_container_svg").remove();
        }

        const scroll_size = chat.scrollTop + chat.clientHeight;

        if (scroll_size >= chat.scrollHeight - 5) {
            const last_item = document.querySelector(".chat .item:last-child");

            if (!last_item) return;

            const creation = last_item.dataset.index;
            const token_message = last_item.id;
            ...
            if (checkScroll && endMessages) {
                ...
                let data = await getMessages(get_messages);
                ...
                if (data.length > 1) {
                    ...
                    data = data.filter(msg => msg.token !== token_message);

                    for (let i = 0; i < data.length; i++) {
                        const el = data[i];

                        if (el.status == '0' && el.key_from_me == '2') continue;
    ...
    $(".iconSendEmailExportTalk").on("click", () => modalIconExportTalk());
    ...
    $('#modal-chat').on('hidden.bs.modal', function () {
        ...
        document.querySelectorAll(".chat .item").forEach(elm => elm.remove());
        document.querySelectorAll(".chat .date-floating").forEach(elm => elm.remove());

        document.getElementById("attendance-name").innerHTML = "";
        document.getElementById("modal-name-contact").innerHTML = "";
        document.getElementById("modal-number-contact").innerHTML = "";
        document.getElementById("modal-profile-user").src = "../../assets/img/avatar.svg";
...
function chatSettings(e) {
    if (modalSettings === true) {
        ...
        export_chat.innerHTML = GLOBAL_LANG.report_call_fuction_chatsetting_export;
...
function AlertNoDataAvailable() {
    ...
    alert_text.innerHTML = GLOBAL_LANG.report_call_chat_no_record_found;
...
function openChat(data, protocol) {
    ...
    document.getElementById("chat-settings")
        .addEventListener("click", chatSettings);

    document.querySelectorAll(".chat .item")
        .forEach(elm => elm.remove());

    document.querySelector("#up_scroll_extra_space")?.remove();

    // 🔎 find protocol index using el.creation
    const protocol_index = data.findIndex(
        el => el.data === protocol
    );
    ...
    if (protocol_index !== -1) {
        ...
        // ⬇️ scroll so protocol becomes first visible line
        setTimeout(() => {
            document.querySelector(".chat").style.overflowY = "scroll";
            const protocol_element = document.querySelector(
                `.chat .item[data-index="${index_creation}"]`
            );

            removeDuplicateIds();
            if (protocol_element) {
                protocol_element.scrollIntoView({
                    behavior: "auto",
                    block: "start"
                });
            }
...
function getMessages(data) {
    return new Promise((resolve, reject) => {
        ...
        if (data.protocol) post_data.protocol = data.protocol

        if (data.creation != null) post_data.creation = data.creation

        $.post("call/getMessages", post_data, (data) => {
            resolve(data);
        });
    });
}
...
async function alertOutNinetyDays() {
    ...
    Swal.fire({
        title: GLOBAL_LANG.report_call_alert_out_ninety_days_title,
        text: GLOBAL_LANG.report_call_service_filter_period_notify,
        type: 'warning',
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: GLOBAL_LANG.report_call_alert_out_ninety_days_confirmButtonText
    });
    ...
    async function cb(start, end, clicked_btn) {
        if (search_period) {
            if (clicked_btn) {
                ...
                const data = await getMessages(post_data);
                if (data != "false") openChat(data, false);
    ...
    $("#reportrange").daterangepicker(
        {
            ...
            locale: {
                "format": "DD/MM/YYYY",
                "separator": " - ",
                "applyLabel": GLOBAL_LANG.report_call_calendar_btn_search,
                "cancelLabel": GLOBAL_LANG.report_call_calendar_btn_return,
                "daysOfWeek": [
                    GLOBAL_LANG.report_call_calendar_week_day_sun,
                    GLOBAL_LANG.report_call_calendar_week_day_mon,
                    GLOBAL_LANG.report_call_calendar_week_day_tue,
                    GLOBAL_LANG.report_call_calendar_week_day_wed,
                    GLOBAL_LANG.report_call_calendar_week_day_thu,
                    GLOBAL_LANG.report_call_calendar_week_day_fri,
                    GLOBAL_LANG.report_call_calendar_week_day_sat
                ],
                "monthNames": [
                    GLOBAL_LANG.report_call_calendar_month_january,
                    GLOBAL_LANG.report_call_calendar_month_february,
                    GLOBAL_LANG.report_call_calendar_month_march,
                    GLOBAL_LANG.report_call_calendar_month_april,
                    GLOBAL_LANG.report_call_calendar_month_may,
                    GLOBAL_LANG.report_call_calendar_month_june,
                    GLOBAL_LANG.report_call_calendar_month_july,
                    GLOBAL_LANG.report_call_calendar_month_august,
                    GLOBAL_LANG.report_call_calendar_month_september,
                    GLOBAL_LANG.report_call_calendar_month_october,
                    GLOBAL_LANG.report_call_calendar_month_november,
                    GLOBAL_LANG.report_call_calendar_month_december
                ]
            },
        }, cb);
...
function statusModal(status) {
    ...
    switch (status) {
        case "1":
            ...
            status_modal.innerHTML = GLOBAL_LANG.report_call_chat_status_in_attendance;
            status_modal.style.top = "-18px";
            status_modal.style.fontSize = "12px";
            break;
        case "2":
            status_modal.className = "diamond bg-green";
            status_modal.innerHTML = GLOBAL_LANG.report_call_chat_status_attendance_closed;
            status_modal.style.top = "-18px";
            status_modal.style.fontSize = "12px";
            break;
    }
...
function modalFilter() {
    ...
    new MSFmultiSelect(
        ...
        searchBox: true,
        width: '100%',
        placeholder: GLOBAL_LANG.report_call_service_filter_channel_placeholder,
    });

    new MSFmultiSelect(
        document.querySelector('#multiselect-categories'), {
        selectAll: true,
        searchBox: true,
        width: '100%',
        placeholder: GLOBAL_LANG.report_call_service_filter_channel_placeholder,
    ...
    new MSFmultiSelect(
        document.querySelector('#multiselect-label'), {
        selectAll: true,
        searchBox: true,
        width: '100%',
        placeholder: GLOBAL_LANG.report_call_service_filter_label_placeholder,
    });
    ...
    new MSFmultiSelect(
        ...
        searchBox: true,
        width: '100%',
        placeholder: GLOBAL_LANG.report_call_service_filter_sector_placeholder,
    });
    ...
    const check_select_categories = document.getElementById("check-select-categories");
    const mult_select_categories = document.getElementById("mult-select-categories");
    const verify_select_categories = document.getElementById("verify-select-categories");

    const check_select_label = document.getElementById("check-select-label");
    const mult_select_label = document.getElementById("mult-select-label");
    const verify_select_label = document.getElementById("verify-select-label");
    ...
    const check_user = document.getElementById("check-user");
    ...
    check_select_categories.addEventListener("click", () => {
        if (check_select_categories.checked) {
            mult_select_categories.style.display = "block";
            verify_select_categories.value = "1";
        }
        else {
            mult_select_categories.style.display = "none";
            verify_select_categories.value = "2";
        }
    });

    check_select_label.addEventListener("click", () => {
        if (check_select_label.checked) {
            mult_select_label.style.display = "block";
            verify_select_label.value = "1";
        }
        else {
            mult_select_label.style.display = "none";
            verify_select_label.value = "2";
        }
    });
    ...
    check_user.addEventListener("click", () => {
    ...
    check_date.addEventListener("click", () => {
        if (check_date.checked) {
            ...
            dt_start.placeholder = GLOBAL_LANG.report_call_service_filter_period_placeholder_date_start;
...
function modalExport() {
    ...
    $.get(`/export/xlsx?
            ...
            &categories=${$('#verify-select-categories').val() == "2" ? "" : $('#multiselect-categories').val()}
            &label=${$('#verify-select-label').val() == "2" ? "" : $('#multiselect-label').val()}
            &sector=${$('#verify-select').val() == "2" ? "" : $('#multiselect').val()}
            &user=${$('#select-user').val()}
            ...
            &dt_start=${$('#dt-start').val()}
            &dt_end=${$('#dt-end').val()}
            &type=reportCall`, function (response) {

        if (response != "Error") {
            Swal.fire({
                title: GLOBAL_LANG.report_call_alert_export_title,
                text: GLOBAL_LANG.report_call_alert_export_text,
                type: 'success',
                reverseButtons: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: GLOBAL_LANG.report_call_alert_export_confirmButtonText
            });
        }
    });
}
...
function modalExportTalk(e) {
    ...
    $.get(`/export/xlsx?
    &id_chat_list=${meuId}
    &dt_start=${$('#dt-start-chat').val()}
    &dt_end=${$('#dt-end-chat').val()}
    &type=reportMessageHistory`, function (response) {

        if (response == "Error") {

            Swal.fire({
                title: GLOBAL_LANG.report_call_service_export_no_permit_title,
                text: GLOBAL_LANG.report_call_service_export_no_permit_content,
                type: 'error',
                reverseButtons: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
...
function modalIconExportTalk(e) {
...
function createTemplateMessage(json) {
    ...
    if (json.components && json.components !== "null") {
        const components = (typeof json.components === 'string')
            ? JSON.parse(json.components)
            : json.components;
        ...
        for (let i = 0; i < components.length; i++) {
            ...
            if (components[i].type === "header") {
                ...
                // HEADER TEXTO (replace {{1}})
                if (json.header && components[i].parameters) {
                    let headerText = json.header;

                    for (let p = 0; p < components[i].parameters.length; p++) {
                        const param = components[i].parameters[p];

                        if (param.type === "text") {
                            headerText = headerText.replaceAll(`{{${p + 1}}}`, param.text);
                        }
                    }
                ...
                if (components[i].parameters?.[0]?.type === "image") {
                    const img = document.createElement("img");
                ...
                if (components[i].parameters?.[0]?.type === "document") {
                    ...
                    title.textContent = GLOBAL_LANG.report_call_document_message;
            ...
            if (components[i].type === "body" && components[i].parameters) {
                let data = json.text_body;

                for (let b = 0; b < components[i].parameters.length; b++) {
                    data = data.replaceAll(`{{${b + 1}}}`, components[i].parameters[b].text);
                }
    ...
    if (json.header) {
        ...
        header_text.textContent = json.header;
        header_text.style.fontWeight = "bold";

        container_header.appendChild(header_text);
...
async function requestOpenAI(media_url) {
    ...
    const errorMsg = GLOBAL_LANG.report_call_chat_transcribe_audio_error_response;
...
async function transcribeAudio(media_url, event) {
    ...
    audioText.innerHTML = GLOBAL_LANG.report_call_chat_audio_text;
    ...
    if (text === "error") {
        ...
        audioText.innerHTML = GLOBAL_LANG.report_call_chat_transcribe_audio_error_apy_key;
...
function setReaction(json) {
    if (json.reaction !== null && json.reaction !== '') {
        ...
        div_reaction_container.className = json.key_from_me == 1 ? "messageLeft reaction-container" : "messageRigth reaction-container";
...
function showUserName(data, attendance_show, set_name_in_chat) {
    if (attendance_show) {
        const chat_attendance_name = document.getElementById("attendance-name");
        chat_attendance_name.innerText = data.user_name != null ? data.user_name : "BOT";

        updateAttendanceName(chat_attendance_name.innerText);

        last_name_attendance = chat_attendance_name.innerText;
    }

    if (set_name_in_chat == 2) return;

    let div_user_name = '';

    switch (data.media_type) {
        case '1':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .body`);
            break;
        case '2':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .audioMessage`);
            break;
        case '3':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .imageMessage`);
            break;
        case '4':
            let el = document.querySelector(`#${CSS.escape(data.token)} .container-document-send`);

            if (el) {
                div_user_name = el;
            } else {
                div_user_name = document.querySelector(`#${CSS.escape(data.token)} .documentMessage`)
            }
            break;
        case '5':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .videoMessage`);
            break;
        case '6':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .gifMessage`);
            break;
        case '7':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .locationMessage`);
            break;
        case '9':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .ContactMessage`);
            break;
        case '10':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .zipMessage`);
            break;
        case '18':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .revokeMessage`);
            break;
        case "19":
        case "20":
        case "21":
        ...
        case "23":
        case "24":
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .information`);
            break;
        case "26":
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .StickerMessage`);
            break;
        case '27':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .templateMessage`);
            break;
        case '30':
        case '35':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .InteractiveMessage`);
            break;
        case '32':
            div_user_name = document.querySelector(`#${CSS.escape(data.token)} .is-reply`);
            break;
    ...
    span.innerText = data.user_name != null ? data.user_name : "BOT";

    div.appendChild(span);

    const firstChild = div_user_name.firstChild;
    div_user_name.insertBefore(div, firstChild);

}

function updateStickerData() {
    const chat_container = document.querySelector('.chat');
    const chat_rect = chat_container.getBoundingClientRect();
    ...
    chat_items.forEach(item => {
        ...
        const is_visible =
            item_rect.bottom > chat_rect.top &&
            item_rect.top < chat_rect.bottom;

        if (is_visible && item_rect.bottom > max_bottom) {
            max_bottom = item_rect.bottom;
            target_item = item;
        }
    });
    ...
    const timestamp = target_item.dataset.index;
    document.querySelector('.date-floating .bottom span').textContent =
        FormatShortDate(timestamp);

    const user_name_el = target_item.querySelector('.user-name');
    if (user_name_el) {
        updateAttendanceName(user_name_el.innerText);
    }
}
function openFlowModal({ title, evaluationNote, comment }) {

    function formatStars(evaluation) {
        if (!evaluation) {
            return '<span style="color: #dee2e6;">☆☆☆☆☆</span>';
        }

        const rating = parseInt(evaluation.split('_').pop());
    ...
    content.innerHTML = `
        ...
        <h3 class="flow-modal-title" style="
            margin: 0 0 20px 0;
            font-size: 20px;
            color: #333;
            font-weight: 700;
        ...
        <div class="modal-section">
            <div class="modal-label">
                ...
                ${GLOBAL_LANG.report_call_modal_assessment}
            </div>
        ...
        <div class="modal-section">
            <div class="modal-label">
                <span class="modal-icon">💬</span>
                ${GLOBAL_LANG.report_call_modal_comment}
            </div>
            <div class="modal-value">
                ${comment || "<i>Nenhum comentário</i>"}
            </div>
        </div>
    `;
...
function updateAttendanceName(name_attendance) {
    const chat_attendance_name = document.getElementById("attendance-name");
    chat_attendance_name.innerText = name_attendance != null ? name_attendance : "BOT";

    const chat_attendance_profile = document.getElementById("modal-profile-user");

    const user = Object.values(data_user).find(
        u => u.last_name === name_attendance || u.name === name_attendance
    );

    chat_attendance_profile.src = "https://files.talkall.com.br:3000/p/" + user?.key_remote_id + ".jpeg" || "../../assets/img/avatar.svg";
}

function parseDatetimeTimestamp(date_str) {
    const [date_part, time_part] = date_str.split(' ');

    const [day, month, year] = date_part.split('/').map(Number);
    const [hour, minute, second] = time_part.split(':').map(Number);
