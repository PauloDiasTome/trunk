...
function FormatShortDate(timestamp) {
    ...
    if (currentDate == dt) {
        return GLOBAL_LANG.report_transfer_automatic_function_fsd_today;
    } else {
        let diff = this.dateDiffInDays(date, today);
        if (diff < 3) {
            switch (diff) {
                case 1:
                    return GLOBAL_LANG.report_transfer_automatic_function_fsd_yesterday;
                case 2:
                    return GLOBAL_LANG.report_transfer_automatic_function_fsd_day_before_yesterday;
            }
        } else {
            if (diff < 7) {
                let semana = [GLOBAL_LANG.report_transfer_automatic_function_fsd_sunday, GLOBAL_LANG.report_transfer_automatic_function_fsd_monday, GLOBAL_LANG.report_transfer_automatic_function_fsd_tuesday, GLOBAL_LANG.report_transfer_automatic_function_fsd_wednesday, GLOBAL_LANG.report_transfer_automatic_function_fsd_thursday, GLOBAL_LANG.report_transfer_automatic_function_fsd_friday, GLOBAL_LANG.report_transfer_automatic_function_fsd_saturday];
...
function message(json) {
    ...
    switch (parseInt(json.media_type)) {
        ...
        case 18:
            if (json.key_from_me == 1) {
                data = GLOBAL_LANG.report_transfer_automatic_function_message_if;
            } else {
                data = GLOBAL_LANG.report_transfer_automatic_function_message_else;
            }
            break;
        case 19:
            data = json.data;
    ...
    switch (parseInt(json.media_type)) {
        ...
        case 20:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_transfer_automatic_parseint_case_20;
            message.appendChild(span);
            item.appendChild(message);
            break;
        case 21:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_transfer_automatic_parseint_case_21;
        ...
        case 22:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_transfer_automatic_parseint_case_22;
        ...
        case 23:
            message.style.background = "transparent";
            var span = document.createElement("span");
            span.innerHTML = GLOBAL_LANG.report_transfer_automatic_parseint_case_23;
...
function find() {
    $('#datatable-basic').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "transfer_automatic/find",
            type: "POST",
            data: {
                text: $("#search").val(),
                user: $('#verify-select').val() == "2" ? "" : $('#multiselect').val(),
        ...
        columnDefs: [
            {
                ...
                render: function (data, type, full, meta) {
                    return "<a href='#' id='" + full.id_chat_list + "' data-name='" + full.name + "' data-number='" + full.number + "' class='table-action table-action-comment' data-toggle='tooltip' data-original-title='" + GLOBAL_LANG.report_transfer_automatic_dt_columndefs_target4_title_edit + "'>" + "<i class='fas fa-comments'></i></a>"
                }
            },
        ],
...
$(document).ready(function () {
    ...
    $("#datatable-basic").on("click", ".table-action-comment", function () {
        ...
        document.getElementById("modal-content-chat").addEventListener("click", () => $(".modal-settings-chat").remove());

        load = true;
        id_chat = this.id;
        statusModal();

        $.post("transfer_automatic/getMessages", {
            id: id_chat,
            creation: 0,
            csrf_talkall: Cookies.get("csrf_cookie_talkall")
        }, (data) => {
...
function chatSettings(e) {
    ...
    wallpaper.innerHTML = GLOBAL_LANG.report_transfer_automatic_function_chatsettings_wallpaper;

    let export_chat = document.createElement("li");
    export_chat.innerHTML = GLOBAL_LANG.report_transfer_automatic_function_chatsettings_export;
    export_chat.dataset.toggle = "modal";
    export_chat.dataset.target = "#modal-export-talk";
...
function statusModal() {
    ...
    status_modal.innerHTML = GLOBAL_LANG.report_transfer_automatic_function_statusmodel_auto_transfer;
}
...
function modalFilter() {
    ...
    var select = new MSFmultiSelect(
        ...
        width: "100%",
        height: 47,
        placeholder: GLOBAL_LANG.report_transfer_automatic_filter_sector_placeholder,
    ...
    check_date.addEventListener("click", () => {
        if (check_date.checked) {
            ...
            dt_start.placeholder = GLOBAL_LANG.report_transfer_automatic_function_modalfilter_data_start;
...
function modalExport() {
    ...
    $.get(`/export/xlsx?
        ...
        &dt_start=${$('#dt-start').val()}
        &dt_end=${$('#dt-end').val()}
        &type=reportAutoTransfer`, function (response) {

        Swal.fire({
            title: GLOBAL_LANG.report_transfer_automatic_alert_export_title,
            text: GLOBAL_LANG.report_transfer_automatic_alert_export_text,
            type: 'success',
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: GLOBAL_LANG.report_transfer_automatic_alert_export_confirmButtonText
        });
    });
}
