...
function checkFilter() {
    if (Filters) {
        ...
        if (Filters.ticket_status.search) {
            document.getElementById("search").value = Filters.ticket_status.search;
        }
                ...
                render: function (data, type, full, meta) {
                    ...
                    return `<div class="dropleft">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="multiDropdownMenu">
                                        <a id="` + full.id_ticket_status + `" href='#' class="dropdown-item table-btn-edit" style="cursor: pointer">
                                            <div style="width: 24px; display: inline-block"> 
                                                <i class="far fa-edit"></i>
                                            </div>
                                            <span>${GLOBAL_LANG.ticket_status_dt_columndefs_target1_title_edit}</span>
                                        </a>
                                        <a id="` + full.id_ticket_status + `" href='#' class="dropdown-item table-btn-delete" style="cursor: pointer">
                                            <div style="width: 24px; display: inline-block"> 
                                                <i class="far fa-trash-alt"></i>
                                            </div>
                                            <span>${GLOBAL_LANG.ticket_status_dt_columndefs_target1_title_delete}</span>
                                        </a>
                                    </div>
                                </div>`
                }
...
$(document).ready(function () {
    ...
    $("#datatable-basic").on("click", ".table-btn-delete", function () {
        swal({
            title: GLOBAL_LANG.ticket_status_alert_delete_title,
            text: GLOBAL_LANG.ticket_status_alert_delete_text,
            type: "warning",
            showCancelButton: !0,
            buttonsStyling: !1,
            reverseButtons: true,
            cancelButtonClass: "btn btn-danger",
            cancelButtonText: GLOBAL_LANG.ticket_status_alert_delete_cancelButtonText,
            confirmButtonClass: "btn btn-success",
            confirmButtonText: GLOBAL_LANG.ticket_status_alert_delete_confirmButtonText,
        }).then(t => {
            if (t.value == true) {
                ...
                $.post("status/delete/" + this.id, function (data) {
                    if (data == true) {
                        t.value && swal({
                            title: GLOBAL_LANG.ticket_status_alert_delete_two_title,
                            text: GLOBAL_LANG.ticket_status_alert_delete_two_text,
                            type: "success",
                            buttonsStyling: !1,
                            confirmButtonClass: "btn btn-success"
                        });
                        ...
                        t.value && swal({
                            title: GLOBAL_LANG.ticket_status_alert_delete_three_title,
                            text: GLOBAL_LANG.ticket_status_alert_delete_three_text,
                            type: "warning",
                            buttonsStyling: !1,
...
function submit() {
    ...
    const input_name = formValidation({ field: document.getElementById("input-name"), text: GLOBAL_LANG.ticket_status_name, required: true, min: 3, max: 100, alpha_numeric_spaces: true });
...
function modalExport() {
    ...
    $.get(`/export/xlsx?
        ...
        if (response != "Error") {
            Swal.fire({
                title: GLOBAL_LANG.ticket_status_alert_export_title,
                text: GLOBAL_LANG.ticket_status_alert_export_text,
                type: 'success',
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: GLOBAL_LANG.ticket_status_alert_export_confirmButtonText
            });
        }
    });
}