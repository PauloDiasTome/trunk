...
function checkFilter() {
    if (Filters) {
        ...
        if (Filters.ticket_subtype.search) {
            document.getElementById("search").value = Filters.ticket_subtype.search;
        }
...
function find() {
    ...
    $('#datatable-basic').DataTable({
        ...
        drawCallback: function (settings) {
            ...
            filter.ticket_subtype = ticket_subtype;
...
$(document).ready(function () {
    ...
    $("#datatable-basic").on("click", ".table-action-access", function () {
        window.location.href = "../ticket/type/subtype/" + this.id;
    });
    ...
    $("#datatable-basic").on("click", ".table-btn-delete", function () {
        swal({
            ...
            if (t.value == true) {
                $.post("delete/" + this.id, function (data) {
                    if (data.errors?.code == "TA-002") {
                        t.value && swal({
                            title: GLOBAL_LANG.ticket_type_alert_delete_three_title,
                            text: GLOBAL_LANG.ticket_type_alert_delete_three_text,
                            type: "warning",
                            buttonsStyling: !1,
                            confirmButtonClass: "btn btn-success"
                        });
...
function modalExport() {
    ...
    let id_type = window.location.pathname.split("/").pop();
    ...
    $.get(`/export/xlsx?
        ...
        &column=${column}
        &order=${order}
        &type=ticketSubtype`, function (response) {
